...
```bash
# 添加 marketplace
claude plugin marketplace add dengd1937/judgment-vault

# 安装插件
claude plugin install judgment-vault@judgment-vault
```

安装后通过 `/judgment-vault [command] [target]` 调用。每次你 push 新代码，用户启动 Claude Code 时会自动拉取更新。
...
通过 `/judgment-vault [command] [target]` 调用，支持六个命令：
