...
description: >
  ...
  /knowledge-vault to: ingest materials (papers, blogs, repos, AI conversations,
...
如果调用了命令但 `~/judgment-vault/` 不存在，提示用户先运行 `/knowledge-vault init`。
...
3. 对于所有其他命令，验证 `~/judgment-vault/` 是否存在。如果不存在，告诉用户先运行 `/knowledge-vault init`，然后停止。
